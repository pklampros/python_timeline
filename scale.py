...
def d3_scale_niceStep(step):
    if step:
        return {
            "floor": lambda x: math.floor(x / step) * step,
            "ceil": lambda x: math.ceil(x / step) * step,
        }
    else:
        return {"floor": lambda x: x, "ceil": lambda x: x}
...
def d3_scale_linearNice(domain, m=None):
    d3_scale_nice(
        domain, d3_scale_niceStep(d3_scale_linearTickRange(domain, m)[2])
    )
    d3_scale_nice(
        domain, d3_scale_niceStep(d3_scale_linearTickRange(domain, m)[2])
    )
    ...
    def range(self, start, stop, step):
        return list(
            map(
                milli2dt,
                range(
                    math.ceil(int(start.timestamp() * 1000) / step) * step,
...
class LinearScale(object):
    def __init__(
        self, domain=None, _range=None, interpolate=None, clamp=False
    ):
        self._domain = [0, 1] if domain is None else domain
        self._range = [0, 1] if _range is None else _range
        self._interpolate = (
            d3_interpolate if interpolate is None else interpolate
        )
        self._clamp = clamp
    ...
    def rescale(self):
        ...
        self._output = linear(
            self._domain, self._range, uninterpolate, self._interpolate
        )
        self._input = linear(
            self._range, self._domain, uninterpolate, d3_interpolate
        )
        return self
    ...
    def copy(self):
        return LinearScale(
            self._domain, self._range, self._interpolate, self._clamp
        )
